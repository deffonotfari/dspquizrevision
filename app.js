...
const SECTION_META = {
  "week-1-signal-basics": "Week 1: Signal Basics",
  "dsp-fundamentals-sampling-frequency": "DSP Fundamentals: Sampling and Frequency",
  "dsp-theory-correlation-convolution": "DSP Theory: Correlation and Convolution",
  "fourier-transform": "Fourier Transform",
  "dsp-architectures": "DSP System Architectures",
  "applications-data-analysis": "Applications: Data Analysis (Audio, Images, Finance)",
  "applications-audio-games": "Applications: Audio for Games"
};
...
async function loadQuestionsJson() {
  // For local dev, use fetch. In production, ensure questions.json is in the same folder.
...
function loadQuestion() {
  if (!questions.length) {
    showError("No questions found for this section. Check that your questions.json uses a valid section key.");
    return;
  }
...
function checkAnswer(q, selectedKey) {
  ...
  buttons.forEach(btn => {
    ...
    if (btnKey === selectedKey && selectedKey !== correctKey) btn.classList.add("wrong");
  });
...
function initResults() {
  ...
  const scopeQs = section ? allQuestions.filter(q => q.section === section) : allQuestions;
  ...
  if ($("accuracy")) $("accuracy").innerText = answered ? Math.round((correct / answered) * 100) : 0;

  const title = $("resultsTitle");
  if (title) {
    if (section) {
      title.innerText = (SECTION_META[section] || section) + " — Results";
      if (mode === "wrong") title.innerText = (SECTION_META[section] || section) + " — Wrong Questions Review";
    } else {
...
(async function main() {
  ...
  if ($("questionText")) {
    if (!currentSection) {
      showError("Missing section in URL. Use quiz.html?section=<section-key>.");
      return;
    }

    const sectionQs = allQuestions.filter(q => q.section === currentSection);
    ...
    if (title) {
      const base = SECTION_META[currentSection] || currentSection;
      title.innerText = quizMode === "wrong" ? `Review Wrong — ${base}` : `Quiz — ${base}`;
    }
